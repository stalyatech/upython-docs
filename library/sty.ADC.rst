...
Usage::
    ...
    adc = sty.ADC(pin)                  # create an analog object from a pin
    val = adc.read()                    # read an analog value

    adc = sty.ADCAll(resolution)        # create an ADCAll object
    adc = sty.ADCAll(resolution, mask)  # create an ADCAll object for selected analog channels
...
.. method:: ADC.read_timed(buf, timer)
   ...
   Example using a Timer object (preferred way)::
       ...
       adc = sty.ADC(sty.Pin.board.VAIN1)  # create an ADC on pin VAIN1
       tim = sty.Timer(6, freq=10)         # create a timer running at 10Hz
...
.. method:: ADC.read_timed_multi((adcx, adcy, ...), (bufx, bufy, ...), timer)
   ...
   *timer* must be a Timer object. The timer must already be initialised
   ...
   Example reading 3 ADC's::
       ...
       adc0 = sty.ADC(sty.Pin.board.VAIN1) # Create ADC's
       adc1 = sty.ADC(sty.Pin.board.VAIN2)
       adc2 = sty.ADC(sty.Pin.board.VAIN3)
       tim = sty.Timer(8, freq=100)        # Create timer
       ...
       sty.ADC.read_timed_multi((adc0, adc1, adc2), (rx0, rx1, rx2), tim)
...
Example::
    ...
    adcall = sty.ADCAll(12, 0x70000) # 12 bit resolution, internal channels
