...
The stalya has an accelerometer (a tiny mass on a tiny spring) that can be used
...
``SENSITIVITY``. Try running this on the stalya and tilt the board left and right
to make the LED turn on and off.

**Exercise: Change the above script so that the blue LED gets brighter the more
you tilt the stalya.  HINT: You will need to rescale the values, intensity goes
from 0-255.**
...
use the ``y()`` value and more LEDs we can turn the stalya into a spirit level. ::
...
particularly sophisticated but it does the job. Run this on your stalya and you
