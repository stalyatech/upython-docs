...
Code
----
By examining the :ref:`stalya_quickref`, we see that ``X1`` is connected to channel 1 of timer 5 (``TIM5 CH1``). Therefore we will first create a ``Timer`` object for timer 5, then create a ``TimerChannel`` object for channel 1::

    from sty import Timer
    from time import sleep

    # timer 5 will be created with a frequency of 100 Hz
    tim = sty.Timer(5, freq=100)
    tchannel = tim.channel(1, Timer.PWM, pin=sty.Pin.board.X1, pulse_width=0)
